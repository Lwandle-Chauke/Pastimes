...
if(isset($_POST['update_product'])){
   ...
   $update_product = $conn->prepare("UPDATE `tblproducts` SET name = ?, category = ?, details = ?, price = ? WHERE id = ?");
   ...
   if(!empty($image)){
      if($image_size > 2000000){
         ...
         $update_image = $conn->prepare("UPDATE `tblproducts` SET image = ? WHERE id = ?");
