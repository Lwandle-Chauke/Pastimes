...
if (isset($_GET['delete'])) {
   ...
   $delete_cart_item = $conn->prepare("DELETE FROM `tblcart` WHERE id = ?");
   $delete_cart_item->execute([$delete_id]);
   header('location:cart.php');
}

if (isset($_GET['delete_all'])) {
   $delete_cart_item = $conn->prepare("DELETE FROM `tblcart` WHERE user_id = ?");
...
if (isset($_POST['update_qty'])) {
   ...
   $update_qty = $conn->prepare("UPDATE `tblcart` SET quantity = ? WHERE id = ?");
...
<section class="shopping-cart">
   ...
   <?php
   $grand_total = 0;
   $select_cart = $conn->prepare("SELECT * FROM `tblcart` WHERE user_id = ?");
   $select_cart->bind_param("i", $user_id); // Binding the user_id parameter
   $select_cart->execute();
   ...
   <form action="" method="POST" class="box">
      ...
      <img src="project images/<?= $fetch_cart['image']; ?>" alt="Product Image">
